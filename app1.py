...
def parse_eu_amount(text: str) -> float:
    ...
    lead_pos = t.startswith("+")
    if lead_neg or lead_pos:
        t = t[1:].strip()
    t = t.replace("€", "").replace(" ", "")
...
def create_app() -> Flask:
    ...
    @app.before_request
    def _trace_requests():
        print(f"TRACE: {request.method} {request.path}  uid={session.get('user_id')!r}")
    ...
    def upload():
        ...
        if request.method == "POST":
            ...
            header_map = {
                "buchungsdatum": "date","datum": "date","wertstellung": "date",
                "buchungstag": "date","belegdatum":"date","valuta":"date",
                "empfaenger":"payee","empfänger":"payee","beguenstigter":"payee","begünstigter":"payee",
                "nameauftraggeber/zahlungspflichtiger":"payee","auftraggeber/name":"payee","name":"payee",
                "verwendungszweck":"purpose","verwendungszweckprimanota":"purpose",
                "verwendungszweck1":"purpose","verwendungszweck2":"purpose","buchungstext":"purpose","beschreibung":"purpose",
    ...
    # --- Debug: Routenübersicht beim Start ---
    ...
    # TODO: hier später echte Kontoverwaltung einbauen
        try:
            return render_template("konten.html")  # falls du schon ein Template hast
        except TemplateNotFound:
        ...
        <div class="container py-4">
          ...
          <p>Stub-Seite. Hier kommt später die Kontoverwaltung hin.</p>
    ...
    def konto_loeschen(konto_id):
        if "user_id" not in session:
            return redirect(url_for("login"))
        with get_db() as conn:
            conn.execute("DELETE FROM buchungen WHERE user_id = ? AND konto_id = ?", (session["user_id"], konto_id))
            conn.execute("DELETE FROM konten WHERE user_id = ? AND id = ?", (session["user_id"], konto_id))
            conn.commit()
    ...
    def set_test():
        ...
        return "set_test done – jetzt /get_test aufrufen"

    @app.route("/get_test")
    def get_test():
        return f"session test_cookie = {session.get('test_cookie')!r}"
    ...
    def reset_all():
        ...
        uid = session["user_id"]
        with get_db() as conn:
        # optional, falls du FK-Constraints nutzt
            conn.execute("PRAGMA foreign_keys = ON")

        # Daten des Users löschen
            conn.execute("DELETE FROM buchungen WHERE user_id = ?", (uid,))
            conn.execute("DELETE FROM import_transaktionen WHERE user_id = ?", (uid,))

        # Falls deine 'regeln'-Tabelle global ist:
        # -> entweder alle löschen ...
            try:
                conn.execute("DELETE FROM regeln")
            except sqlite3.OperationalError:
                pass
        # ... oder (empfohlen) regeln auf user_id umstellen und dann:
        # conn.execute("DELETE FROM regeln WHERE user_id = ?", (uid,))

        # Konten mitlöschen (falls gewünscht)
