from flask import Blueprint, app, redirect, url_for, render_template, request, flash, session
import sqlite3
import io
import csv
import pandas as pd
...
from app.services.booking_service import berechne_steuer
...
# -------------------------- Konten -----------------------------------------
@main.route("/konten", methods=["GET", "POST"])
def konten():
        if "user_id" not in session:
            ...
            if request.method == "POST":
                ...
                if name and typ in {"einnahme", "ausgabe", "neutral"}:
                    try:
                        ...
                        return redirect(url_for("main.konten"))
                    except sqlite3.IntegrityError:
...
@main.route("/konten/<int:konto_id>")
...
@main.route("/konten/<int:konto_id>/bearbeiten", methods=["GET", "POST"])
        ...
        with get_db() as conn:
            ...
            if request.method == "POST":
                ...
                if not neuer_name or neuer_typ not in {"einnahme", "ausgabe", "neutral"}:
                    ...
                    return redirect(url_for("main.konto_bearbeiten", konto_id=konto_id))
                try:
                    conn.execute(
                    ...
                    return redirect(url_for("main.konto_details", konto_id=konto_id))
                except sqlite3.IntegrityError:
                    flash("❌ Kontonummer bereits vergeben.")
                    return redirect(url_for("main.konto_bearbeiten", konto_id=konto_id))
        return render_template("konto_bearbeiten.html", konto=konto)

@main.route("/konten/<int:konto_id>/loeschen", methods=["POST"])
...
# -------------------------- Regeln -----------------------------------------
@main.route("/regeln", methods=["GET", "POST"])
def regeln():
        ...
        with get_db() as conn:
            if request.method == "POST":
                ...
                if not suchbegriff or not konto_id:
                    ...
                    return redirect(url_for("main.regeln"))
                try:
                    conn.execute(
...
@main.route("/regeln/<int:regel_id>/loeschen", methods=["POST"])
...
@main.route("/regeln/<int:regel_id>/bearbeiten", methods=["GET", "POST"])
...
@main.route("/zuordnung_loeschen", methods=["POST"])
...
# -------------------------- Zuordnen ----------------------------------------
@main.route("/zuordnen", methods=["GET", "POST"])
def zuordnen():
        if "user_id" not in session:
            flash("⛔ Bitte zuerst einloggen.")
            return redirect(url_for("main.login"))
...
def kontakte():
    ...
    # -------------------------- Buchungen --------------------------------------
@main.route("/buchungen", methods=["GET"])
def buchungen_liste():
        if "user_id" not in session:
            flash("⛔ Bitte zuerst einloggen.")
            return redirect(url_for("main.login"))
...
@main.route("/buchungen/loeschen", methods=["POST"], endpoint="buchungen_loeschen")
def buchungen_loeschen():
        ...
        try:
            ...
            return redirect(url_for("main.buchungen_liste"))
...
@main.route("/buchungen/<int:buchung_id>/bearbeiten", methods=["GET", "POST"])
...
@main.route("/buchungen/<int:buchung_id>/steuer-korrigieren", methods=["POST"])
...
@main.route("/buchungen/neu", methods=["GET", "POST"])
def buchung_neu():
        if "user_id" not in session:
           flash("⛔ Bitte einloggen.")
           return redirect(url_for("main.login"))
...
@main.route("/anfragen/status", methods=["POST"])
def anfragen_status():
        if "user_id" not in session:
            return redirect(url_for("login"))
